...
public class RatingRepository(IDbConnectionFactory dbConnectionFactory) : IRatingRepository
{
    ...
    public async Task<bool> DeleteRatingAsync(Guid movieId, Guid userId, CancellationToken ct = default)
    {
        using var connection = await dbConnectionFactory.CreateConnectionAsync(ct);
        var result = await connection.ExecuteAsync(
            new CommandDefinition(
    ...
    public async Task<IEnumerable<MovieRating>> GetRatingsForUserAsync(Guid userId, CancellationToken ct = default)
    {
        using var connection = await dbConnectionFactory.CreateConnectionAsync(ct);
